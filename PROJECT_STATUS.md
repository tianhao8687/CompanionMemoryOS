...
模式合并、身份/时间纠正、双向阶段变化、关系编译、自动 Runtime 接入、CLI 与版本日志。
当前默认解释器采用可替换的保守规则；阶段阈值为可配置的原型策略，尚未进行真人校准。
312 项测试通过（新增 33 项），格式、Ruff、严格 mypy 和依赖检查通过；CLI 本地输入及 wheel 独立安装检查通过。
按要求不进行真实模型调用或人格质量实验。使用方法见 [v0.2 关系模型说明](docs/COMPANION_AGENT_0.2.md)。

## CompanionAgent v0.1 — 2026-09-13

稳定人格层功能已完成：人格校验/加载/编译、独立预算、关系阶段、角色背景和种子、
MemoryOS 上下文组合、Main LLM 适配器、连续对话持久化及版本日志。
279 项功能/回归测试通过；Ruff、严格 mypy、wheel 资源和安装后 CLI 检查通过。
按用户要求未开展真实模型、长对话人格或跨模型实验。
实现范围和使用方法见 [CompanionAgent 说明](docs/COMPANION_AGENT_0.1.md)。

以下保留 MemoryOS 0.7.5 基线状态。

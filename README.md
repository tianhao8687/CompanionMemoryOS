...
打开 **http://127.0.0.1:8765**，点击「认识一下」或左下角「我的陪伴设置」，选择相处设定，
开启本机保存和当前对话模式处理授权，即可使用默认的离线演示。离线演示采用固定规则，
用于验证功能流程，不代表真实模型的聊天质量。
需要连接模型时，在「对话方式」切换为 API 模式，填写 Key 与服务支持的模型名称。
已有虚拟环境时也可运行 `powershell -ExecutionPolicy Bypass -File .\start-romance.ps1`。
Key 不写入数据库或浏览器存储。默认仅在本次运行中使用；Windows 可勾选“在这台电脑记住 Key”，使用当前 Windows 账号的凭据管理器保存并在重启后自动读取。保存按程序数据目录和 API 地址隔离，取消勾选并保存或点击清除 Key 可移除；测试实例不读取系统凭据。
...
参见 [修订、对照回放与实际验证记录](docs/SEMANTIC_REVISION.md)。真实模型体验尚未验证。

补充修复了条件后件、动作先后顺序和否定转述的作用域，避免不确定内容改变持久状态。
见 [小范围语义修复与验证](docs/SEMANTIC_SCOPES.md)。

以下为前序版本记录；其表达策略以 v0.4.1 修订说明为准。

**CompanionAgent v0.4 Current State 已接入**：短期处境与沟通要求可跨轮延续，本轮新要求即时覆盖，
过期或源失效后停止沿用。复用关系动态和 OpenLoop，不改写身份、人格或共同历史。
全量 372 项测试通过。见 [接入说明与可复现回放](docs/CURRENT_STATE.md)。

**当前新增能力：CompanionAgent v0.3 经历层与关系语义修正。** 关系身份、熟悉度、当前距离独立；
支持第一天确认恋人身份，同时保持 NEW 和空共同历史。多轮 Episode 可形成用户/角色/共同经历，
支持合并、生命周期、召回、证据下钻，并作为熟悉度和里程碑的上游。
全量 339 项测试通过。参见 [v0.3 技术与接入说明](docs/COMPANION_AGENT_0.3.md)。

以下保留前序版本完成记录。

**CompanionAgent v0.2 关系模型已完成**：`agent.chat(request)` 自动使用有证据的关系身份、
阶段、互动模式、里程碑、未解决事项、边界和变化历史。新增 33 项关系测试，
全量 312 项通过；未进行真实模型实验。参见 [v0.2 接入说明](docs/COMPANION_AGENT_0.2.md)。
...
新增默认离线演示、明确偏好的自动记忆与更正遗忘、事件关心、工具审批后续跑、微信文本/模拟消息入口、文字流及浏览器语音。先完成实现，再集中测试；本轮不连接真实 DeepSeek。运行和验收步骤见 [功能说明](docs/FUNCTIONAL_COMPANION.md)。

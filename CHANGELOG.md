...
## CompanionAgent 0.4.0 - 2026-09-14

- 新增短期 Current State，按语义和作用域延续或退出；支持当轮要求覆盖、否定纠正、独立事项和时间到期。
- 将有效状态接入原 `agent.chat(request)` 的回应目标、记忆使用计划、人格和最终上下文，持续倾听时收住未经请求的角色往事。
- 复用关系动态处理有对象的冲突与修复，复用 OpenLoop 处理唯一匹配的事项结果，不重置熟悉度或长期边界。
- 按来源时间/序号保护迟到写入，处理回执避免重试延长期限；可选状态组件失败时降级到原路径。
- 来源会话的引用反馈现在能约束跨会话派生上下文；被限制的动态不再暗中决定当前表达距离。
- 新增 CLI 状态检查、配置开关、连续上下文回放与文档。新增 33 项测试，全量 372 项通过。
